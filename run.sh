...
export HYDROSAT_TURBIDITY_PRIOR_SHRINK="${HYDROSAT_TURBIDITY_PRIOR_SHRINK:-0.05}"
export HYDROSAT_CHLA_PRIOR_SHRINK="${HYDROSAT_CHLA_PRIOR_SHRINK:-0}"
export HYDROSAT_NEUTRALIZE_GEO="${HYDROSAT_NEUTRALIZE_GEO:-0}"
export HYDROSAT_TURBIDITY_MODE="${HYDROSAT_TURBIDITY_MODE:-model}"
export HYDROSAT_TURBIDITY_INVERT_RANK="${HYDROSAT_TURBIDITY_INVERT_RANK:-0}"
export HYDROSAT_TURBIDITY_HEURISTIC_WEIGHT="${HYDROSAT_TURBIDITY_HEURISTIC_WEIGHT:-0.81}"
export HYDROSAT_TURBIDITY_CALIBRATION="${HYDROSAT_TURBIDITY_CALIBRATION:-lognormal_rank}"
export HYDROSAT_TURBIDITY_LOGNORMAL_SIGMA="${HYDROSAT_TURBIDITY_LOGNORMAL_SIGMA:-0.52}"
...
"${PYTHON_BIN}" -c "import os, runpy, sys; sys.path.insert(0, os.environ['HYDROSAT_SRC_DIR']); runpy.run_module('hydrosat.infer', run_name='__main__')" --input-root "${INPUT_DIR:-/input}" --output-dir "${OUTPUT_DIR:-/output}" --model-dir "${MODEL_DIR:-/workspace/artifacts/models}" --patch-size "${PATCH_SIZE:-24}" --progress-every "${PROGRESS_EVERY:-1000}"
